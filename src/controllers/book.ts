...
import Payprocess from '../models/payprocess'
...
const getBookComment = (req : Request, res : Response, next : NextFunction) => {
  ...
  const postPayProcess = (req : Request, res : Response, next : NextFunction) => {
    Payprocess.find({email : req.params.email})
    .exec()
    .then(results =>{
        // 200-> 성공 했다면
        return res.status(200).json({
...
export default {getBook, getBookDetail, getBookComment, postPayProcess}
